      ...
      create or replace  table DEV_ANALYTICS.ANALYTICS.global_superstore_summary_fact copy grants as
      (
...
(
    ...
    left join DEV_ANALYTICS.analytics.global_sales_returns returns on fact.order_id = returns.order_id and fact.region = returns.region
    left join DEV_ANALYTICS.master_data.global_sales_person_dim sp on sp.sales_person_region = fact.region
...
select
    ...
    varchar
), '') || '-' || coalesce(cast(sd.state as 
    varchar
), '') || '-' || coalesce(cast(country as 
    varchar
), '') || '-' || coalesce(cast(sd.fiscal_month as 
    varchar
), '') as 
    varchar
))  as global_summary_fact_key,
    sd.fiscal_year,
    sd.fiscal_month,
